<?php
/**
 * Regression tests for Citex_AI_V2's APA Book MCQ generation path — mirrors
 * tests/ai-v2-mla-book-mcq-variant-wiring.test.php's own structure, but for
 * APA's own curated 8-variant catalogue (Citex_APA_Book_Mcq_Variants):
 * Gemini supplies only the canonical record (authorFullNames/year/bookTitle/
 * publisher — no `place` field at all), normalise_apa_book_mcq_variant_item()
 ...
 * the entire question deterministically via Citex_APA_Book_Mcq_Variants::build().
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-apa-book-mcq-variant-wiring.test.php` — not shipped in
 * citex-tools.zip.
 */
...
function invoke_normalise( $questions, $ids, $target_count = null ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), 'MCQ', Citex_Reference_Rules::CATEGORY_BOOK, $target_count, '', '', 'full_reference', 'apa' );
}

function apa_mcq_item( $author_names, $suffix, $publisher = 'Routledge' ) {
	return array(
...
$known_variants = Citex_APA_Book_Mcq_Variants::variants();

// ---------------------------------------------------------------------
// 1. A single-author question is fully Citex-authored: source/category,
// mcqPattern, apaBookMcqVariant, and an exact match against
// Citex_APA_Book_Mcq_Variants::build()'s own output for that variant and
// this record's fields.
// ---------------------------------------------------------------------
$single_item   = apa_mcq_item( array( 'John Smith' ), 'One' );
$single_result = invoke_normalise( array( $single_item ), array( 'AB01' ) );
check( '[1] normalise() succeeds for a single-author APA Book MCQ item', is_wp_error( $single_result ), false );
if ( ! is_wp_error( $single_result ) ) {
	...
	check( '[1] mcqPattern is apa_book_mcq_variant', $candidate['mcqPattern'], 'apa_book_mcq_variant' );
	check( '[1] apaBookMcqVariant is one of the 8 known variants', in_array( $candidate['apaBookMcqVariant'], $known_variants, true ), true );
	...
	check( '[1] author carries no givenName field', array_key_exists( 'givenName', $candidate['authors'][0] ), false );
	...
	$expected = Citex_APA_Book_Mcq_Variants::build(
		$candidate['apaBookMcqVariant'],
		array( 'authors' => array( array( 'surname' => 'Smith', 'initials' => 'J.', 'fullName' => 'John Smith' ) ), 'year' => '2021', 'title' => 'Book One', 'publisher' => 'Routledge' )
	);
...
$batch_ids   = array();
for ( $i = 1; $i <= 30; $i++ ) {
	$batch_items[] = apa_mcq_item( array( 'John Smith' ), (string) $i, diverse_publisher( $i ) );
...
if ( ! is_wp_error( $batch_result ) ) {
	$variants_seen = array_unique( array_column( $batch_result, 'apaBookMcqVariant' ) );
	...
	foreach ( $batch_result as $candidate ) {
		if ( ! in_array( $candidate['apaBookMcqVariant'], $known_variants, true ) ) {
			check( '[2] every candidate\'s apaBookMcqVariant is a known variant', $candidate['apaBookMcqVariant'], '(unknown)' );
		}
	}
}
...
function check_variant_compatible_with_count( $section, $author_full_names, $target_count ) {
	...
	$ids   = array();
	for ( $i = 1; $i <= 20; $i++ ) {
		$items[] = apa_mcq_item( $author_full_names, (string) $i, diverse_publisher( $i ) );
	...
	foreach ( $result as $candidate ) {
		$bounds = Citex_APA_Book_Mcq_Variants::variant_author_requirement( $candidate['apaBookMcqVariant'] );
...
// to Journal Article's field-variety mechanism) — APA Book MCQ candidates
// never carry it.
// ---------------------------------------------------------------------
if ( ! is_wp_error( $single_result ) ) {
	check( '[4] APA Book MCQ candidates carry no exerciseDesign field at all', array_key_exists( 'exerciseDesign', $single_result[0] ), false );
}
