 ...
 * Citex_Generated_Validator::validate_mhra_book_mcq_variant() — the MHRA
 * Book MCQ counterpart to validate_book_mcq_variant()/validate_chicago_book_mcq_variant(),
 * dispatched on a candidate's `mcqPattern` field, backing MHRA's own
 * curated 8-variant catalogue (Citex_MHRA_Book_Mcq_Variants). Mirrors
 * tests/generated-validator-chicago-book-mcq-variant.test.php's own
 * structure.
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-mhra-book-mcq-variant.test.php` — not
...
// directly from Citex_MHRA_Book_Mcq_Variants::build() itself rather than
...
$built = Citex_MHRA_Book_Mcq_Variants::build( 'complete_reference', $fields );

function mhra_book_variant_question( $built, $fields, $overrides = array() ) {
	return array_merge(
		array(
			'source'                 => 'MHRA',
			'group'                  => 'ReferenceList',
			'category'               => 'Book',
			'type'                   => 'MCQ',
			'mcqPattern'             => 'mhra_book_mcq_variant',
			'mhraBookMcqVariant'     => 'complete_reference',
			'scenario'               => $built['stem'],
...
// ---------------------------------------------------------------------
$result = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields ) );
check( '[1] a correctly-built mhra_book_mcq_variant question passes', $result['status'], 'passed' );
...
// ---------------------------------------------------------------------
$bad_count = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'options' => array( 'a', 'b', 'c' ) ) ) );
...
// ---------------------------------------------------------------------
$empty_option = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'options' => array( '', 'x', 'y', '' ) ) ) );
...
// ---------------------------------------------------------------------
$fourth_not_blank = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'options' => array( 'x', 'y', 'z', 'not blank' ) ) ) );
...
// ---------------------------------------------------------------------
$duplicate_options = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'options' => array( 'x', 'x', 'y', '' ) ) ) );
...
// ---------------------------------------------------------------------
$missing_answer = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'reconstructedReference' => '' ) ) );
...
// ---------------------------------------------------------------------
$option_matches_answer = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array(
...
// ---------------------------------------------------------------------
// 8. CRITICAL — an unrecognised mhraBookMcqVariant fails.
// ---------------------------------------------------------------------
$unknown_variant = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'mhraBookMcqVariant' => 'nonexistent_variant' ) ) );
check( '[8] an unrecognised mhraBookMcqVariant fails', $unknown_variant['status'], 'failed' );
check( '[8] reports MHRA_BOOK_MCQ_VARIANT_UNKNOWN', has_error_code( $unknown_variant, 'mhra_book_mcq_variant_unknown' ), true );
...
// ---------------------------------------------------------------------
$wrong_stem = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'scenario' => 'Some other question text entirely.' ) ) );
check( '[9] a mismatched scenario fails', $wrong_stem['status'], 'failed' );
check( '[9] reports MHRA_BOOK_MCQ_VARIANT_STEM_MISMATCH', has_error_code( $wrong_stem, 'mhra_book_mcq_variant_stem_mismatch' ), true );

// ---------------------------------------------------------------------
// 10. CRITICAL — an answer that is not EXACTLY the variant's own
// recomputed correct answer fails, even if it sounds plausible.
// ---------------------------------------------------------------------
$wrong_answer = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array(
	'reconstructedReference' => 'Smith, John, Digital Culture (Oxford: Oxford University Press, 2021).',
) ) );
check( '[10] an answer that is not exactly the variant\'s own answer fails', $wrong_answer['status'], 'failed' );
check( '[10] reports MHRA_BOOK_MCQ_VARIANT_ANSWER_MISMATCH', has_error_code( $wrong_answer, 'mhra_book_mcq_variant_answer_mismatch' ), true );

// ---------------------------------------------------------------------
// 11. CRITICAL — an option that does not exactly match the variant's own
// recomputed wrong option fails.
// ---------------------------------------------------------------------
$wrong_option = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array(
	'options' => array( 'Smith, John, Something else entirely (London: Routledge, 2021).', $built['wrongOptions'][1], $built['wrongOptions'][2], '' ),
) ) );
check( '[11] a tampered option fails', $wrong_option['status'], 'failed' );
check( '[11] reports MHRA_BOOK_MCQ_VARIANT_OPTION_MISMATCH', has_error_code( $wrong_option, 'mhra_book_mcq_variant_option_mismatch' ), true );

// ---------------------------------------------------------------------
// 12. Missing hint fails.
// ---------------------------------------------------------------------
$missing_hint = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array( 'hint' => '' ) ) );
...
// ---------------------------------------------------------------------
$hint_reveals = Citex_Generated_Validator::validate( mhra_book_variant_question( $built, $fields, array(
...
// id, and correctly exercises MHRA's "only the first author inverted,
// natural word order after, always list everyone" rule end-to-end.
// ---------------------------------------------------------------------
...
$three_author_built  = Citex_MHRA_Book_Mcq_Variants::build( 'three_or_more_author_joining', $three_author_fields );
$three_author_result = Citex_Generated_Validator::validate( mhra_book_variant_question( $three_author_built, $three_author_fields, array( 'mhraBookMcqVariant' => 'three_or_more_author_joining' ) ) );
