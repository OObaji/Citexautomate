<?php
/**
 * Regression tests for Citex_Generated_Validator's MHRA in-text citation
 * wiring — validate_intext_dragdrop()/validate_mhra_intext_mcq_variant(),
 * dispatched off the `group`/`mcqPattern`/`source` fields
 * Citex_AI_V2::normalise_intext_item() now produces for style 'mhra'.
 * Mirrors tests/generated-validator-apa-intext.test.php's own structure.
 *
 * Positive coverage: every one of the 24 combinations (4 categories x 3
 * forms x 2 types) Citex_AI_V2::normalise() produces for MHRA in-text
 * citation validates as 'passed'. Negative coverage: deliberately
 * corrupting one field at a time makes validate() FAIL — including a
 * comma wrongly OMITTED before the year (MHRA's own rule requires one —
 * the OPPOSITE of Chicago's own no-comma rule) and a "p." prefix wrongly
 * omitted before a page number.
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-mhra-intext.test.php` — not shipped in
 * citex-tools.zip.
 */
...
// ---------------------------------------------------------------------
// 4. CRITICAL — a comma wrongly OMITTED before the year (MHRA's own rule
// requires one, the OPPOSITE of Chicago's own no-comma rule) must fail
// validation.
// ---------------------------------------------------------------------
...
if ( ! is_wp_error( $parenthetical_sample ) ) {
	...
	check( '[4] a comma wrongly omitted before the year fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[4] a parenthetical sample was available for the comma check', false, true );
}

// ---------------------------------------------------------------------
// 5. CRITICAL — a "p." prefix wrongly omitted before a page number (MHRA
// always requires one, the OPPOSITE of Chicago's own no-"p." rule) must
// fail validation.
// ---------------------------------------------------------------------
...
if ( ! is_wp_error( $quote_sample ) ) {
	$broken = $quote_sample;
	$page   = $broken['page'];
	$broken['reconstructedReference'] = str_replace( 'p. ' . $page . ')', $page . ')', $broken['reconstructedReference'] );
	check( '[5] a "p." prefix wrongly omitted before the page fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[5] a direct-quote sample was available for the "p." prefix check', false, true );
}

// ---------------------------------------------------------------------
// 6. A missing year for a non-Website category must fail validation.
// ---------------------------------------------------------------------
...
if ( ! is_wp_error( $narrative_sample ) ) {
	...
	check( '[6] a year wrongly omitted from an MHRA in-text citation fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
