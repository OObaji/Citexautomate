<?php
/**
 * Regression tests for Citex_AI_V2's Chicago (Author-Date) Phase 2
 * generation path (Edited Book, Journal Article, Website) — mirrors
 * tests/ai-v2-apa-phase2-wiring.test.php's own structure, exercising all 3
 * new Chicago categories' DragDrop AND MCQ normalisers in one file, since
 * each category's own Chicago-Book-Phase-1 wiring test already proved the
 * "Citex authors the entire question, Gemini supplies only the canonical
 * record" pattern works end-to-end — this file's job is proving that
 * pattern now also holds for Edited Book/Journal Article/Website, WITH the
 * `place` field Chicago keeps throughout (unlike APA/MLA).
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-chicago-phase2-wiring.test.php` — not shipped in
 * citex-tools.zip.
 */
...
function invoke_normalise( $questions, $ids, $category, $type ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), $type, $category, null, '', '', 'full_reference', 'chicago' );
}

// Deliberately different-length place/publisher pools — see
// ai-v2-chicago-book-dragdrop-wiring.test.php's own docblock for why they
// must never cycle in lockstep within a batch.
...
for ( $i = 1; $i <= 12; $i++ ) {
	$eb_items[] = array(
		'scenario'        => "You are referencing an edited book titled Digital Culture $i, edited by Amy Ross, published in 2019 by " . diverse_publisher( $i ) . ' in ' . diverse_place( $i ) . '.',
		'editorFullNames' => array( 'Amy Ross' ),
		'year'            => '2019',
		'bookTitle'       => "Digital Culture $i",
		'place'           => diverse_place( $i ),
		'publisher'       => diverse_publisher( $i ),
	);
	$eb_ids[] = 'CE' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $eb_dd_result ) ) {
	...
	foreach ( $eb_dd_result as $candidate ) {
		if ( 'Chicago' !== $candidate['source'] || 'Edited Book' !== $candidate['category'] || ! array_key_exists( 'place', $candidate ) || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[EditedBook DragDrop] every candidate is Chicago/Edited Book, HAS a place field, and validates', $all_valid, true );
...
if ( ! is_wp_error( $eb_mcq_result ) ) {
	...
	foreach ( $eb_mcq_result as $candidate ) {
		if ( 'chicago_edited_book_mcq_variant' !== $candidate['mcqPattern'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[EditedBook MCQ] every candidate has mcqPattern chicago_edited_book_mcq_variant and validates', $all_valid, true );
}
...
for ( $i = 1; $i <= 12; $i++ ) {
	...
	$ja_ids[] = 'CJ' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $ja_dd_result ) ) {
	...
	foreach ( $ja_dd_result as $candidate ) {
		if ( 'Chicago' !== $candidate['source'] || 'Journal Article' !== $candidate['category'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[JournalArticle DragDrop] every candidate is Chicago/Journal Article and validates', $all_valid, true );
}
...
if ( ! is_wp_error( $ja_mcq_result ) ) {
	...
	foreach ( $ja_mcq_result as $candidate ) {
		if ( 'chicago_journal_article_mcq_variant' !== $candidate['mcqPattern'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[JournalArticle MCQ] every candidate has mcqPattern chicago_journal_article_mcq_variant and validates', $all_valid, true );
}
...
for ( $i = 1; $i <= 10; $i++ ) {
	...
	$web_ids[]   = 'CW' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $web_dd_result ) ) {
	...
	foreach ( $web_dd_result as $candidate ) {
		if ( 'Chicago' !== $candidate['source'] || array_key_exists( 'accessedDate', $candidate ) || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[Website DragDrop] every candidate is Chicago, has no accessedDate field, and validates', $all_valid, true );
}
...
if ( ! is_wp_error( $web_mcq_result ) ) {
	...
	foreach ( $web_mcq_result as $candidate ) {
		if ( 'chicago_website_mcq_variant' !== $candidate['mcqPattern'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[Website MCQ] every candidate has mcqPattern chicago_website_mcq_variant and validates', $all_valid, true );
}
