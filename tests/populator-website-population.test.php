 ...
 * the new Website/Web Resource category — proves populate_one() creates a
 * real WordPress post, assigns the real "Website"/"Exercise N" taxonomy
 * terms (found purely by NAME, via the same find_taxonomy_term_by_name()
 * mechanism already proven category-agnostic for Book/Journal
 * Article/Website in tests/populator-category-exercise-assignment.test.php),
 * writes the generic fixedText/questionParts/confusingWords/scenario ACF
 * fields Citex always writes regardless of category, and verifies the
 * saved post before reporting success — with ZERO Populator code changes
 * required, since Citex_Populator never branches on category at all (see
 * its own docblock).
 *
 * Repo-level only, run with plain
 * `php tests/populator-website-population.test.php` — not shipped in
 * citex-tools.zip.
 */
...
// Website (id 30) with its own Exercise 1-5 children (ids 31-35) — a real
// WordPress taxonomy shape, exactly like Book's/Journal Article's in the
// sibling tests.
$GLOBALS['__terms_full']['reference_category'] = array(
	30 => array( 'name' => 'Website', 'parent' => 0 ),
	31 => array( 'name' => 'Exercise 1', 'parent' => 30 ),
	32 => array( 'name' => 'Exercise 2', 'parent' => 30 ),
	33 => array( 'name' => 'Exercise 3', 'parent' => 30 ),
	34 => array( 'name' => 'Exercise 4', 'parent' => 30 ),
	35 => array( 'name' => 'Exercise 5', 'parent' => 30 ),
);
...
$question = array(
	'key'            => 'k-wr',
	'questionId'     => 'WR01',
	'title'          => 'Harvard | ReferenceList | Website | DragDrop | WR01',
	'category'       => 'Website',
	'exercise'       => 'Exercise 4',
	'type'           => 'DragDrop',
	...
	'confusingWords' => array( '2020', 'A different publisher', 'https://example.com/wrong' ),
	'scenario'       => 'You are referencing a webpage written by Sarah Mitchell in 2024 and published by the University of Leeds.',
...
// ---------------------------------------------------------------------
// 26, 27, 28 & 29. Website + Exercise 4 + DragDrop: populate_one() creates
// the post, assigns BOTH real taxonomy terms (Category="Website",
// Exercise="Exercise 4"), writes the generic ACF fields, and verifies
// everything actually persisted before reporting success — the same
// finalisation mechanism Book/Edited Book/Journal Article already use,
// with zero Populator code changes.
// ---------------------------------------------------------------------
check( '[26][27][28] populate_one() succeeds for Website with no Populator code changes', is_wp_error( $result ), false );
if ( ! is_wp_error( $result ) ) {
	$post_id = $result['postId'];
	$saved_terms = $GLOBALS['__post_terms'][ $post_id ]['reference_category'] ?? array();
	check( '[26] the "Website" category term (30) is assigned', in_array( 30, $saved_terms, true ), true );
	check( '[27] the "Exercise 4" term (34), Website\'s own child, is assigned', in_array( 34, $saved_terms, true ), true );
	check( '[29] result reports the verified Category', $result['category'], 'Website' );
	check( '[29] result reports the verified Exercise', $result['exercise'], 'Exercise 4' );
	check( '[29] result reports categoryVerified', $result['categoryVerified'], true );
	check( '[29] result reports exerciseVerified', $result['exerciseVerified'], true );
	check( '[28] the Fixed Text ACF field was written', $GLOBALS['__acf_values'][ $post_id ][ Citex_Populator::FIELD_FIXED_TEXT ] ?? null, '| (||) || [online]. ||. Available from: <||> [accessed ||].' );
	check( '[28] the Question Parts repeater field was written with all 6 parts', count( $GLOBALS['__acf_values'][ $post_id ][ Citex_Populator::FIELD_QUESTION_PARTS ] ?? array() ), 6 );
}

// ---------------------------------------------------------------------
// A representative 5-exercise x {DragDrop, MCQ} matrix is correctly
// classified for Website — proves the taxonomy-assignment mechanism
// handles the full Website coverage matrix (item 22's exercise-coverage
// requirement) with zero Populator changes.
// ---------------------------------------------------------------------
$exercise_term_ids = array( 'Exercise 1' => 31, 'Exercise 2' => 32, 'Exercise 3' => 33, 'Exercise 4' => 34, 'Exercise 5' => 35 );
$batch_ok = true;
$post_id_seq = 500;
foreach ( $exercise_term_ids as $exercise_name => $expected_term_id ) {
	foreach ( array( 'DragDrop', 'MCQ' ) as $type ) {
		$classification = invoke_private( $populator, 'resolve_classification', array( array( 'category' => 'Website', 'exercise' => $exercise_name, 'type' => $type ) ) );
		$GLOBALS['__posts'][ $post_id_seq ] = array( 'post_type' => 'question', 'post_status' => 'draft', 'post_title' => 'x', 'post_content' => '', 'post_excerpt' => '', 'menu_order' => 0 );
		$assign = invoke_private( $populator, 'assign_generated_classification', array( $post_id_seq, 'question', $classification ) );
		if ( is_wp_error( $assign ) ) { $batch_ok = false; break 2; }
		$saved = $GLOBALS['__post_terms'][ $post_id_seq ]['reference_category'] ?? array();
		if ( array( 30, $expected_term_id ) !== $saved ) { $batch_ok = false; break 2; }
		$post_id_seq++;
	}
}
check( '[22][26][27] a full 10-slot Website matrix (5 exercises x {DragDrop, MCQ}) is all correctly classified', $batch_ok, true );
check( 'exactly 10 slots were exercised', $post_id_seq - 500, 10 );
