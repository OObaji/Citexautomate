 ...
 * Citex_Generated_Validator::validate_mla_book_mcq_variant() — the MLA Book
 * MCQ counterpart to validate_book_mcq_variant(), dispatched on a
 * candidate's `mcqPattern` field, backing MLA's own curated 8-variant
 * catalogue (Citex_MLA_Book_Mcq_Variants). Mirrors
 * tests/generated-validator-book-mcq-variant.test.php's own structure
 * exactly.
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-mla-book-mcq-variant.test.php` — not
...
// directly from Citex_MLA_Book_Mcq_Variants::build() itself rather than
...
$built = Citex_MLA_Book_Mcq_Variants::build( 'complete_reference', $fields );

function mla_book_variant_question( $built, $fields, $overrides = array() ) {
	return array_merge(
		array(
			'source'                 => 'MLA',
			'group'                  => 'ReferenceList',
			'category'               => 'Book',
			'type'                   => 'MCQ',
			'mcqPattern'             => 'mla_book_mcq_variant',
			'mlaBookMcqVariant'      => 'complete_reference',
			'scenario'               => $built['stem'],
...
// ---------------------------------------------------------------------
$result = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields ) );
check( '[1] a correctly-built mla_book_mcq_variant question passes', $result['status'], 'passed' );
...
// ---------------------------------------------------------------------
$bad_count = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'options' => array( 'a', 'b', 'c' ) ) ) );
...
// ---------------------------------------------------------------------
$empty_option = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'options' => array( '', 'x', 'y', '' ) ) ) );
...
// ---------------------------------------------------------------------
$fourth_not_blank = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'options' => array( 'x', 'y', 'z', 'not blank' ) ) ) );
...
// ---------------------------------------------------------------------
$duplicate_options = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'options' => array( 'x', 'x', 'y', '' ) ) ) );
...
// ---------------------------------------------------------------------
$missing_answer = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'reconstructedReference' => '' ) ) );
...
// ---------------------------------------------------------------------
$option_matches_answer = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array(
...
// ---------------------------------------------------------------------
// 8. CRITICAL — an unrecognised mlaBookMcqVariant fails.
// ---------------------------------------------------------------------
$unknown_variant = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'mlaBookMcqVariant' => 'nonexistent_variant' ) ) );
check( '[8] an unrecognised mlaBookMcqVariant fails', $unknown_variant['status'], 'failed' );
check( '[8] reports MLA_BOOK_MCQ_VARIANT_UNKNOWN', has_error_code( $unknown_variant, 'mla_book_mcq_variant_unknown' ), true );
...
// ---------------------------------------------------------------------
$wrong_stem = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'scenario' => 'Some other question text entirely.' ) ) );
check( '[9] a mismatched scenario fails', $wrong_stem['status'], 'failed' );
check( '[9] reports MLA_BOOK_MCQ_VARIANT_STEM_MISMATCH', has_error_code( $wrong_stem, 'mla_book_mcq_variant_stem_mismatch' ), true );

// ---------------------------------------------------------------------
// 10. CRITICAL — an answer that is not EXACTLY the variant's own
// recomputed correct answer fails, even if it sounds plausible.
// ---------------------------------------------------------------------
$wrong_answer = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array(
	'reconstructedReference' => 'Smith, John. Digital Culture. Oxford University Press, 2021.',
) ) );
check( '[10] an answer that is not exactly the variant\'s own answer fails', $wrong_answer['status'], 'failed' );
check( '[10] reports MLA_BOOK_MCQ_VARIANT_ANSWER_MISMATCH', has_error_code( $wrong_answer, 'mla_book_mcq_variant_answer_mismatch' ), true );

// ---------------------------------------------------------------------
// 11. CRITICAL — an option that does not exactly match the variant's own
// recomputed wrong option fails.
// ---------------------------------------------------------------------
$wrong_option = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array(
	'options' => array( 'Smith, John. Something Else Entirely. Routledge, 2021.', $built['wrongOptions'][1], $built['wrongOptions'][2], '' ),
) ) );
check( '[11] a tampered option fails', $wrong_option['status'], 'failed' );
check( '[11] reports MLA_BOOK_MCQ_VARIANT_OPTION_MISMATCH', has_error_code( $wrong_option, 'mla_book_mcq_variant_option_mismatch' ), true );

// ---------------------------------------------------------------------
// 12. Missing hint fails.
// ---------------------------------------------------------------------
$missing_hint = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array( 'hint' => '' ) ) );
...
// ---------------------------------------------------------------------
$hint_reveals = Citex_Generated_Validator::validate( mla_book_variant_question( $built, $fields, array(
...
// and correctly exercises MLA's "et al." rule end-to-end.
// ---------------------------------------------------------------------
...
$three_author_built  = Citex_MLA_Book_Mcq_Variants::build( 'et_al_convention', $three_author_fields );
$three_author_result = Citex_Generated_Validator::validate( mla_book_variant_question( $three_author_built, $three_author_fields, array( 'mlaBookMcqVariant' => 'et_al_convention' ) ) );
