<?php
/**
 * Regression tests for Citex_AI_V2's MLA Journal Article MCQ generation
 * path — mirrors tests/ai-v2-mla-book-mcq-variant-wiring.test.php's own
 * structure exactly, but for Citex_MLA_Journal_Article_Mcq_Variants:
 * Gemini supplies only the canonical record (authorFullNames/articleTitle/
 * journalTitle/volume/issue/year/pages — no place/publisher at all),
 * normalise_mla_journal_article_mcq_item() picks a variant per QUESTION
 * (seeded by that question's own id, filtered to variants compatible with
 * the record's real author count) and builds the entire question
 * deterministically via Citex_MLA_Journal_Article_Mcq_Variants::build().
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-mla-journal-article-mcq-variant-wiring.test.php` — not
...
function invoke_normalise( $questions, $ids, $target_count = null ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), 'MCQ', Citex_Reference_Rules::CATEGORY_JOURNAL_ARTICLE, $target_count, '', '', 'full_reference', 'mla' );
}

function mla_ja_mcq_item( $author_names, $suffix ) {
	return array(
...
$known_variants = Citex_MLA_Journal_Article_Mcq_Variants::variants();

// ---------------------------------------------------------------------
// 1. A single-author question is fully Citex-authored: source/category,
// mcqPattern, mlaJournalArticleMcqVariant, and an exact match against
// Citex_MLA_Journal_Article_Mcq_Variants::build()'s own output.
// ---------------------------------------------------------------------
$single_item   = mla_ja_mcq_item( array( 'John Smith' ), 'One' );
$single_result = invoke_normalise( array( $single_item ), array( 'MJ01' ) );
check( '[1] normalise() succeeds for a single-author MLA Journal Article MCQ item', is_wp_error( $single_result ), false );
if ( ! is_wp_error( $single_result ) ) {
	...
	check( '[1] mcqPattern is mla_journal_article_mcq_variant', $candidate['mcqPattern'], 'mla_journal_article_mcq_variant' );
	check( '[1] mlaJournalArticleMcqVariant is one of the 8 known variants', in_array( $candidate['mlaJournalArticleMcqVariant'], $known_variants, true ), true );
	...
	$expected = Citex_MLA_Journal_Article_Mcq_Variants::build(
		$candidate['mlaJournalArticleMcqVariant'],
		array(
			'authors'      => array( array( 'surname' => 'Smith', 'givenName' => 'John', 'fullName' => 'John Smith' ) ),
			'articleTitle' => 'Art One',
			'journalTitle' => 'J. Media One',
...
$batch_ids   = array();
for ( $i = 1; $i <= 30; $i++ ) {
	$batch_items[] = mla_ja_mcq_item( array( 'John Smith' ), (string) $i );
...
if ( ! is_wp_error( $batch_result ) ) {
	$variants_seen = array_unique( array_column( $batch_result, 'mlaJournalArticleMcqVariant' ) );
	...
	foreach ( $batch_result as $candidate ) {
		if ( ! in_array( $candidate['mlaJournalArticleMcqVariant'], $known_variants, true ) ) {
			check( '[2] every candidate\'s mlaJournalArticleMcqVariant is a known variant', $candidate['mlaJournalArticleMcqVariant'], '(unknown)' );
		}
	}
}
...
function check_variant_compatible_with_count( $section, $author_full_names, $target_count ) {
	...
	$ids   = array();
	for ( $i = 1; $i <= 20; $i++ ) {
		$items[] = mla_ja_mcq_item( $author_full_names, (string) $i );
	...
	foreach ( $result as $candidate ) {
		$bounds = Citex_MLA_Journal_Article_Mcq_Variants::variant_author_requirement( $candidate['mlaJournalArticleMcqVariant'] );
...
// only to Harvard Journal Article's field-variety mechanism.
// ---------------------------------------------------------------------
if ( ! is_wp_error( $single_result ) ) {
	check( '[4] MLA Journal Article MCQ candidates carry no exerciseDesign field at all', array_key_exists( 'exerciseDesign', $single_result[0] ), false );
}
