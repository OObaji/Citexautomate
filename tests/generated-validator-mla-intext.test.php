<?php
/**
 * Regression tests for Citex_Generated_Validator's MLA in-text citation
 * wiring — validate_intext_dragdrop()/validate_mla_intext_mcq_variant(),
 * dispatched off the `group`/`mcqPattern`/`source` fields
 * Citex_AI_V2::normalise_intext_item() now produces.
 *
 * Positive coverage: every one of the 24 combinations (4 categories x 3
 * forms x 2 types) Citex_AI_V2::normalise() produces for MLA in-text
 ...
 * single most-tested MLA-vs-Harvard distractor (a comma wrongly inserted
 * before the page in the quote form), proving the validator actually
 * rejects the Harvard mistake when it leaks into an MLA record.
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-mla-intext.test.php` — not shipped in
 * citex-tools.zip.
 */
...
function build_candidate( $category, $form, $type, $n ) {
...
// ---------------------------------------------------------------------
// 4. The single most-tested MLA-vs-Harvard distractor: a hand-corrupted
// quote-form DragDrop candidate whose reconstructedReference wrongly
// inserts a comma before the page (the Harvard mistake) must fail
// validation, never pass.
// ---------------------------------------------------------------------
$quote_sample = build_candidate( $BOOK, 'parenthetical_quote', 'DragDrop', 9002 );
if ( ! is_wp_error( $quote_sample ) ) {
	$broken = $quote_sample;
	$who    = $broken['authors'][0]['surname'] ?? '';
	$broken['reconstructedReference'] = '"' . trim( (string) $broken['quote'] ) . '" (' . $who . ', ' . $broken['page'] . ').';
	check( '[4] a comma wrongly inserted before the page (the Harvard mistake) fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[4] a quote-form sample was available for the regression check', false, true );
}

// ---------------------------------------------------------------------
// 5. A year wrongly injected into an MLA in-text citation (MLA never
// shows one) must fail validation, never pass.
// ---------------------------------------------------------------------
$narrative_sample = build_candidate( $BOOK, 'narrative', 'DragDrop', 9003 );
if ( ! is_wp_error( $narrative_sample ) ) {
	$broken = $narrative_sample;
	$who    = $broken['authors'][0]['surname'] ?? '';
	$broken['reconstructedReference'] = $who . ' (2020) ' . $broken['clause'] . ' (' . $broken['page'] . ').';
	check( '[5] a year wrongly injected into an MLA citation fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[5] a narrative sample was available for the year-injection check', false, true );
}
