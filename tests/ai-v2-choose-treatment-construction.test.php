...
if ( ! is_wp_error( $result ) ) {
	...
	check( '[1] the stem matches the user\'s own confirmed wording', $candidate['scenario'], 'Which statement is correct about a book with four or more authors in the reference list?' );
...
if ( ! is_wp_error( $two_authors_result ) ) {
	check( '[2] two_authors correct statement', $two_authors_result[0]['reconstructedReference'], 'Both authors are included, joined by "and".' );
}
...
if ( ! is_wp_error( $three_authors_result ) ) {
	check( '[2] three_authors correct statement', $three_authors_result[0]['reconstructedReference'], 'All three authors are included, separated by commas with "and" before the final author.' );
}
...
if ( ! is_wp_error( $two_editors_result ) ) {
	...
	check( '[3] two_editors correct statement', $two_editors_result[0]['reconstructedReference'], 'Both editors are included, joined by "and", followed by the designation "(eds)".' );
}
...
if ( ! is_wp_error( $three_or_more_editors_result ) ) {
	check( '[3] three_or_more_editors correct statement', $three_or_more_editors_result[0]['reconstructedReference'], 'All editors are included, separated by commas with "and" before the final editor, followed by the designation "(eds)".' );
}
