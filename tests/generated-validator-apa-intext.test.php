<?php
/**
 * Regression tests for Citex_Generated_Validator's APA in-text citation
 * wiring — validate_intext_dragdrop()/validate_apa_intext_mcq_variant(),
 * dispatched off the `group`/`mcqPattern`/`source` fields
 * Citex_AI_V2::normalise_intext_item() now produces for style 'apa'.
 * Mirrors tests/generated-validator-mla-intext.test.php's own structure.
 *
 * Positive coverage: every one of the 24 combinations (4 categories x 3
 * forms x 2 types) Citex_AI_V2::normalise() produces for APA in-text
 ...
 * single most APA-distinctive mistake, the wrong joiner for the citation
 * form ("&" wrongly used in a narrative citation, or "and" wrongly used in
 * a parenthetical one).
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-apa-intext.test.php` — not shipped in
 * citex-tools.zip.
 */
...
// ---------------------------------------------------------------------
// 4. CRITICAL — the single most APA-distinctive in-text mistake: the wrong
// joiner for the citation form. A 2-author narrative citation using "&"
// (the parenthetical joiner) instead of "and" must fail validation, and a
// 2-author parenthetical citation using "and" instead of "&" must fail too.
// ---------------------------------------------------------------------
$narrative_two = build_candidate( $BOOK, 'narrative', 'DragDrop', 9001, 2 );
if ( ! is_wp_error( $narrative_two ) ) {
	$broken = $narrative_two;
	$broken['reconstructedReference'] = str_replace( ' and ', ' & ', $broken['reconstructedReference'] );
	check( '[4] "&" wrongly used in a narrative citation fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[4] a 2-author narrative sample was available for the joiner check', false, true );
}

$parenthetical_two = build_candidate( $BOOK, 'parenthetical', 'DragDrop', 9002, 2 );
if ( ! is_wp_error( $parenthetical_two ) ) {
	$broken = $parenthetical_two;
	$broken['reconstructedReference'] = str_replace( ' & ', ' and ', $broken['reconstructedReference'] );
	check( '[4] "and" wrongly used in a parenthetical citation fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[4] a 2-author parenthetical sample was available for the joiner check', false, true );
}

// ---------------------------------------------------------------------
// 5. A missing year for a non-Website category (APA always requires one,
// unlike MLA's optional year) must fail validation.
// ---------------------------------------------------------------------
...
if ( ! is_wp_error( $narrative_sample ) ) {
	...
	check( '[5] a year wrongly omitted from an APA in-text citation fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[5] a narrative sample was available for the year-omission check', false, true );
}
