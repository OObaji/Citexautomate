<?php
/**
 * Regression tests for Citex_AI_V2's MHRA Phase 2 generation path (Edited
 * Book, Journal Article, Website) — mirrors
 * tests/ai-v2-chicago-phase2-wiring.test.php's own structure, exercising
 * all 3 new MHRA categories' DragDrop AND MCQ normalisers in one file,
 * since each category's own MHRA-Book-Phase-1 wiring test already proved
 * the "Citex authors the entire question, Gemini supplies only the
 ...
 * WITH the `place` field MHRA keeps throughout, and — the single biggest
 * structural difference from every other style's own Website generation
 * path — NO `year` field at all for Website (only `accessedDate`).
 *
 * Repo-level only, run with plain `php tests/ai-v2-mhra-phase2-wiring.test.php`
...
function invoke_normalise( $questions, $ids, $category, $type ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), $type, $category, null, '', '', 'full_reference', 'mhra' );
}

// Deliberately different-length place/publisher pools — see
// ai-v2-mhra-book-dragdrop-wiring.test.php's own docblock for why they must
// never cycle in lockstep within a batch.
...
for ( $i = 1; $i <= 12; $i++ ) {
	...
	$eb_ids[] = 'HE' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $eb_dd_result ) ) {
	...
	foreach ( $eb_dd_result as $candidate ) {
		if ( 'MHRA' !== $candidate['source'] || 'Edited Book' !== $candidate['category'] || ! array_key_exists( 'place', $candidate ) || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[EditedBook DragDrop] every candidate is MHRA/Edited Book, HAS a place field, and validates', $all_valid, true );
...
if ( ! is_wp_error( $eb_mcq_result ) ) {
	...
	foreach ( $eb_mcq_result as $candidate ) {
		if ( 'mhra_edited_book_mcq_variant' !== $candidate['mcqPattern'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[EditedBook MCQ] every candidate has mcqPattern mhra_edited_book_mcq_variant and validates', $all_valid, true );
}
...
for ( $i = 1; $i <= 12; $i++ ) {
	...
	$ja_ids[] = 'HJ' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $ja_dd_result ) ) {
	...
	foreach ( $ja_dd_result as $candidate ) {
		if ( 'MHRA' !== $candidate['source'] || 'Journal Article' !== $candidate['category'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[JournalArticle DragDrop] every candidate is MHRA/Journal Article and validates', $all_valid, true );
}
...
if ( ! is_wp_error( $ja_mcq_result ) ) {
	...
	foreach ( $ja_mcq_result as $candidate ) {
		if ( 'mhra_journal_article_mcq_variant' !== $candidate['mcqPattern'] || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[JournalArticle MCQ] every candidate has mcqPattern mhra_journal_article_mcq_variant and validates', $all_valid, true );
}

// ---------------------------------------------------------------------
// Website — DragDrop and MCQ, individual + organisation. No `year` field
// at all — see Citex_MHRA_Reference_Rules's own docblock.
// ---------------------------------------------------------------------
function website_item( $i, $individual ) {
	$who  = $individual ? 'John Smith' : 'World Health Organisation';
	$item = array(
...
for ( $i = 1; $i <= 10; $i++ ) {
	...
	$web_ids[]   = 'HW' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
if ( ! is_wp_error( $web_dd_result ) ) {
	...
	foreach ( $web_dd_result as $candidate ) {
		if ( 'MHRA' !== $candidate['source'] || array_key_exists( 'year', $candidate ) || ! array_key_exists( 'accessedDate', $candidate ) || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[Website DragDrop] every candidate is MHRA, has NO year field but HAS accessedDate, and validates', $all_valid, true );
}
...
if ( ! is_wp_error( $web_mcq_result ) ) {
	...
	foreach ( $web_mcq_result as $candidate ) {
		if ( 'mhra_website_mcq_variant' !== $candidate['mcqPattern'] || array_key_exists( 'year', $candidate ) || 'passed' !== Citex_Generated_Validator::validate( $candidate )['status'] ) {
			$all_valid = false;
		}
	}
	check( '[Website MCQ] every candidate has mcqPattern mhra_website_mcq_variant, no year field, and validates', $all_valid, true );
}
