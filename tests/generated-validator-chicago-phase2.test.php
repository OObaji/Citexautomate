 ...
 * Phase 2 DragDrop/MCQ blocks (Edited Book, Journal Article, Website) —
 * mirrors tests/generated-validator-apa-phase2.test.php's own "tamper one
 * field, confirm the exact-match check catches it" pattern, consolidated
 * across all 3 new categories in one file, WITH the `place` field Chicago
 * keeps throughout (unlike APA).
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-chicago-phase2.test.php` — not shipped in
 * citex-tools.zip.
 */
...
require __DIR__ . '/../citex-tools/includes/class-citex-chicago-edited-book-dragdrop-parts.php';
require __DIR__ . '/../citex-tools/includes/class-citex-chicago-edited-book-mcq-variants.php';
require __DIR__ . '/../citex-tools/includes/class-citex-chicago-journal-article-dragdrop-parts.php';
require __DIR__ . '/../citex-tools/includes/class-citex-chicago-journal-article-mcq-variants.php';
require __DIR__ . '/../citex-tools/includes/class-citex-chicago-website-dragdrop-parts.php';
...
$eb_keys    = Citex_Chicago_Edited_Book_Dragdrop_Parts::select_parts( 'CE01', $eb_editors );
$eb_built   = Citex_Chicago_Edited_Book_Dragdrop_Parts::build( $eb_keys, $eb_editors, $eb_fields );
$eb_ref     = Citex_Chicago_Reference_Rules::build_reference( Citex_Chicago_Reference_Rules::CATEGORY_EDITED_BOOK, array_merge( $eb_fields, array( 'editors' => $eb_editors ) ) );
function eb_question( $keys, $built, $editors, $fields, $ref, $overrides = array() ) {
	return array_merge(
		array(
			'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Edited Book', 'type' => 'DragDrop',
...
check( '[EB DD] reports CHICAGO_EDITED_BOOK_DRAGDROP_FIXED_TEXT_MISMATCH', has_error_code( $eb_tampered, 'chicago_edited_book_dragdrop_fixed_text_mismatch' ), true );

// ---------------------------------------------------------------------
// Edited Book MCQ.
// ---------------------------------------------------------------------
$eb_mcq_fields = array( 'editors' => $eb_editors, 'year' => '2019', 'title' => 'Urban Planning Today', 'place' => 'London', 'publisher' => 'Routledge' );
$eb_variant    = Citex_Chicago_Edited_Book_Mcq_Variants::variant_for( 'CE01', 1 );
$eb_mcq_built  = Citex_Chicago_Edited_Book_Mcq_Variants::build( $eb_variant, $eb_mcq_fields );
function eb_mcq_question( $variant, $built, $editors, $fields, $overrides = array() ) {
	return array_merge(
		array(
			'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Edited Book', 'type' => 'MCQ', 'mcqPattern' => 'chicago_edited_book_mcq_variant',
			'chicagoEditedBookMcqVariant' => $variant, 'scenario' => $built['stem'], 'editors' => $editors, 'year' => $fields['year'], 'bookTitle' => $fields['title'], 'place' => $fields['place'], 'publisher' => $fields['publisher'],
...
check( '[EB MCQ] reports CHICAGO_EDITED_BOOK_MCQ_VARIANT_OPTION_MISMATCH', has_error_code( $eb_mcq_tampered, 'chicago_edited_book_mcq_variant_option_mismatch' ), true );
...
$ja_keys     = Citex_Chicago_Journal_Article_Dragdrop_Parts::select_parts( 'CJ01', $ja_authors );
$ja_built    = Citex_Chicago_Journal_Article_Dragdrop_Parts::build( $ja_keys, $ja_authors, $ja_fields );
$ja_ref      = Citex_Chicago_Reference_Rules::build_reference( Citex_Chicago_Reference_Rules::CATEGORY_JOURNAL_ARTICLE, array_merge( $ja_fields, array( 'authors' => $ja_authors ) ) );
...
$ja_question = array(
	'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Journal Article', 'type' => 'DragDrop',
...
check( '[JA DD] reports CHICAGO_JOURNAL_ARTICLE_DRAGDROP_PARTS_MISMATCH', has_error_code( $ja_tampered, 'chicago_journal_article_dragdrop_parts_mismatch' ), true );

// CRITICAL — an APA-shaped reference (no quotes around the article title,
// "pp."-free but ALSO no colon before the pages) for a Chicago-sourced
// Journal Article record fails the Chicago-specific format check. Uses a
// selection that draws only "year" (never "article_title"), so the real
// quoted title sits as LITERAL text in fixedText — the one place a
// str_replace() on that literal text can actually take effect.
$ja_apa_shaped_keys  = array( 'year' );
$ja_apa_shaped_built = Citex_Chicago_Journal_Article_Dragdrop_Parts::build( $ja_apa_shaped_keys, $ja_authors, $ja_fields );
check( '[JA DD setup] the literal quoted title is present in fixedText before tampering', false !== strpos( $ja_apa_shaped_built['fixedText'], '"Climate Change and Policy."' ), true );
$ja_apa_shaped_question = array(
	'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Journal Article', 'type' => 'DragDrop',
	'scenario' => $ja_scenario,
	'dragdropPartKeys' => $ja_apa_shaped_keys,
	'fixedText' => str_replace( '"Climate Change and Policy."', 'Climate Change and Policy.', $ja_apa_shaped_built['fixedText'] ),
	'questionParts' => $ja_apa_shaped_built['parts'],
	'confusingWords' => $ja_apa_shaped_built['confusingWords'],
...
$ja_apa_shaped_result = Citex_Generated_Validator::validate( $ja_apa_shaped_question );
check( '[JA DD] an unquoted (APA-style) article title for a Chicago record fails', $ja_apa_shaped_result['status'], 'failed' );
check( '[JA DD] reports CHICAGO_JOURNAL_ARTICLE_FORMAT_MISMATCH', has_error_code( $ja_apa_shaped_result, 'chicago_journal_article_format_mismatch' ), true );
...
$ja_variant      = Citex_Chicago_Journal_Article_Mcq_Variants::variant_for( 'CJ01', 1 );
$ja_mcq_built    = Citex_Chicago_Journal_Article_Mcq_Variants::build( $ja_variant, $ja_mcq_fields );
$ja_mcq_question = array(
	'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Journal Article', 'type' => 'MCQ', 'mcqPattern' => 'chicago_journal_article_mcq_variant',
	'chicagoJournalArticleMcqVariant' => $ja_variant, 'scenario' => $ja_mcq_built['stem'], 'authors' => $ja_authors, 'year' => $ja_fields['year'], 'articleTitle' => $ja_fields['articleTitle'], 'journalTitle' => $ja_fields['journalTitle'], 'volume' => $ja_fields['volume'], 'issue' => $ja_fields['issue'], 'pages' => $ja_fields['pages'],
...
check( '[JA MCQ] reports CHICAGO_JOURNAL_ARTICLE_MCQ_VARIANT_ANSWER_MISMATCH', has_error_code( $ja_mcq_tampered, 'chicago_journal_article_mcq_variant_answer_mismatch' ), true );
...
$web_keys     = Citex_Chicago_Website_Dragdrop_Parts::select_parts( 'CW01' );
$web_built    = Citex_Chicago_Website_Dragdrop_Parts::build( $web_keys, $web_author, $web_fields );
$web_ref      = Citex_Chicago_Reference_Rules::build_reference( Citex_Chicago_Reference_Rules::CATEGORY_WEBSITE, array_merge( $web_fields, array( 'author' => $web_author ) ) );
$web_scenario = 'You are referencing a webpage titled ' . $web_fields['title'] . ' by John Smith, at ' . $web_fields['url'] . '.';
$web_question = array(
	'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Website', 'type' => 'DragDrop',
...
check( '[Web DD] reports CHICAGO_WEBSITE_DRAGDROP_CONFUSING_WORDS_MISMATCH', has_error_code( $web_tampered, 'chicago_website_dragdrop_confusing_words_mismatch' ), true );

// CRITICAL — a Harvard/APA-shaped ("Available at:") reference for a
// Chicago Website record fails the Chicago-specific format check, never
// silently passing as if it were correct.
$web_harvard_shaped = array_merge( $web_question, array( 'reconstructedReference' => 'Smith, John 2020. "Understanding Climate Policy." Available at: https://example.com.' ) );
$web_harvard_result = Citex_Generated_Validator::validate( $web_harvard_shaped );
check( '[Web DD] a Harvard/APA-shaped ("Available at:") reference for a Chicago record fails', $web_harvard_result['status'], 'failed' );

$web_mcq_fields   = array( 'author' => $web_author, 'year' => $web_fields['year'], 'title' => $web_fields['title'], 'url' => $web_fields['url'] );
$web_variant      = Citex_Chicago_Website_Mcq_Variants::variant_for( 'CW01' );
$web_mcq_built    = Citex_Chicago_Website_Mcq_Variants::build( $web_variant, $web_mcq_fields );
$web_mcq_question = array(
	'source' => 'Chicago', 'group' => 'ReferenceList', 'category' => 'Website', 'type' => 'MCQ', 'mcqPattern' => 'chicago_website_mcq_variant',
	'chicagoWebsiteMcqVariant' => $web_variant, 'scenario' => $web_mcq_built['stem'], 'authorType' => 'individual', 'authors' => array( $web_author ), 'year' => $web_fields['year'], 'pageTitle' => $web_fields['title'], 'url' => $web_fields['url'],
...
$web_mcq_tampered = Citex_Generated_Validator::validate( array_merge( $web_mcq_question, array( 'chicagoWebsiteMcqVariant' => 'not_a_real_variant' ) ) );
check( '[Web MCQ] an unrecognised variant id fails', $web_mcq_tampered['status'], 'failed' );
check( '[Web MCQ] reports CHICAGO_WEBSITE_MCQ_VARIANT_UNKNOWN', has_error_code( $web_mcq_tampered, 'chicago_website_mcq_variant_unknown' ), true );
