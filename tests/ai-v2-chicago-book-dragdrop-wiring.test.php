<?php
/**
 * Regression tests for Citex_AI_V2's Chicago (Author-Date) Book DragDrop
 * generation path — mirrors tests/ai-v2-apa-book-dragdrop-wiring.test.php's
 * own structure, but for Chicago's own rule (Citex_Chicago_Book_Dragdrop_Parts):
 * a `place` field IS carried (like Harvard, unlike MLA/APA), a full given
 * name (`givenName`, the SAME shape MLA's own wiring uses, never Harvard/
 * APA's `initials`), every author always listed in full ("and" before the
 * last, comma before it even at exactly 2, never "et al."), and any of the
 ...
 * no fixedText, no confusingWords. normalise_chicago_book_dragdrop_item()
 * picks a selection per QUESTION (seeded by that question's own id) and
 * builds the entire question deterministically via
 * Citex_Chicago_Book_Dragdrop_Parts::select_parts()/build().
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-chicago-book-dragdrop-wiring.test.php` — not shipped in
 * citex-tools.zip.
 */
...
function invoke_normalise( $questions, $ids, $target_count = null ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), 'DragDrop', Citex_Reference_Rules::CATEGORY_BOOK, $target_count, '', '', 'full_reference', 'chicago' );
}

function chicago_dragdrop_item( $author_names, $suffix, $place = 'London', $publisher = 'Routledge' ) {
	return array(
...
// LCM 42) so the (place, publisher) PAIR never repeats within a 30-question
// batch — using two same-length pools would silently re-pair the same two
// values every 7th question, tripping check_place_publisher_diversity()'s
// own "same pair repeated" hard rule for reasons unrelated to what section
// [4] below actually tests.
...
// Citex_Chicago_Book_Dragdrop_Parts::build()'s own recomputation, and it
// round-trips through Citex_Generated_Validator::validate().
// ---------------------------------------------------------------------
$single_item   = chicago_dragdrop_item( array( 'John Smith' ), 'One', 'London', 'Routledge' );
$single_result = invoke_normalise( array( $single_item ), array( 'CB01' ) );
check( '[1] normalise() succeeds for a single-author Chicago Book DragDrop item', is_wp_error( $single_result ), false );
if ( ! is_wp_error( $single_result ) ) {
	$candidate = $single_result[0];
	check( '[1] source is Chicago', $candidate['source'], 'Chicago' );
	...
	$expected_built   = Citex_Chicago_Book_Dragdrop_Parts::build( $candidate['dragdropPartKeys'], $expected_authors, $expected_fields );
	check( '[1] Question Parts are exactly Citex_Chicago_Book_Dragdrop_Parts::build()\'s own output', $candidate['questionParts'], $expected_built['parts'] );
	check( '[1] Fixed Text is exactly Citex_Chicago_Book_Dragdrop_Parts::build()\'s own output', $candidate['fixedText'], $expected_built['fixedText'] );
	check( '[1] confusingWords is exactly Citex_Chicago_Book_Dragdrop_Parts::build()\'s own output', $candidate['confusingWords'], $expected_built['confusingWords'] );
	check( '[1] reconstructedReference matches build_reference()', $candidate['reconstructedReference'], 'Smith, John. 2019. Book One. London: Routledge.' );
...
$batch_ids   = array();
for ( $i = 1; $i <= 30; $i++ ) {
	$batch_items[] = chicago_dragdrop_item( array( 'John Smith' ), (string) $i, diverse_place( $i ), diverse_publisher( $i ) );
	$batch_ids[]   = 'CB' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
// ---------------------------------------------------------------------
// 3. Multi-author records (2, 3, 4 authors — Chicago always lists everyone
// in full, joined with "and") generate and validate correctly too — the
// "and" candidate is exercised across a spread of seeds for each count.
// ---------------------------------------------------------------------
function check_multi_author( $section, $author_full_names ) {
	$items = array();
	$ids   = array();
	for ( $i = 1; $i <= 20; $i++ ) {
		$items[] = chicago_dragdrop_item( $author_full_names, $section . $i, diverse_place( $i ), diverse_publisher( $i ) );
		$ids[]   = 'CB' . str_pad( $i, 2, '0', STR_PAD_LEFT ) . $section;
	}
...
for ( $i = 1; $i <= 10; $i++ ) {
	$repetitive_items[] = chicago_dragdrop_item( array( 'John Smith' ), 'R' . $i, 'London', 'Routledge' );
	$repetitive_ids[]   = 'CBR' . $i;
}
...
for ( $i = 1; $i <= 10; $i++ ) {
	$genuinely_diverse_items[] = chicago_dragdrop_item( array( 'John Smith' ), 'V' . $i, diverse_place( $i ), diverse_publisher( $i ) );
	$genuinely_diverse_ids[]   = 'CBV' . $i;
}
