<?php
/**
 * Regression tests for Citex_AI_V2's APA Book DragDrop generation path —
 * mirrors tests/ai-v2-mla-book-dragdrop-wiring.test.php's own structure,
 * but for APA's own rule (Citex_APA_Book_Dragdrop_Parts): no `place` field
 * at all, initials (never a full given name, the SAME shape Harvard's own
 * wiring uses), every author always listed in full ("&" before the last,
 * never "et al."), and any of the authors can be the one split into
 * draggable pieces (unlike MLA, which only ever draws the first).
 *
 ...
 * fixedText, no confusingWords. normalise_apa_book_dragdrop_item() picks a
 * selection per QUESTION (seeded by that question's own id) and builds the
 * entire question deterministically via
 * Citex_APA_Book_Dragdrop_Parts::select_parts()/build().
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-apa-book-dragdrop-wiring.test.php` — not shipped in
 * citex-tools.zip.
 */
...
function invoke_normalise( $questions, $ids, $target_count = null ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), 'DragDrop', Citex_Reference_Rules::CATEGORY_BOOK, $target_count, '', '', 'full_reference', 'apa' );
}

function apa_dragdrop_item( $author_names, $suffix, $publisher = 'Routledge' ) {
	return array(
...
// `place` field at all, author carries initials (not givenName),
// dragdropPartKeys/questionParts/fixedText/confusingWords exactly match
// Citex_APA_Book_Dragdrop_Parts::build()'s own recomputation, and it
// round-trips through Citex_Generated_Validator::validate().
// ---------------------------------------------------------------------
$single_item   = apa_dragdrop_item( array( 'John Smith' ), 'One' );
$single_result = invoke_normalise( array( $single_item ), array( 'AB01' ) );
check( '[1] normalise() succeeds for a single-author APA Book DragDrop item', is_wp_error( $single_result ), false );
if ( ! is_wp_error( $single_result ) ) {
	$candidate = $single_result[0];
	check( '[1] source is APA', $candidate['source'], 'APA' );
	...
	check( '[1] author carries initials, not givenName', $candidate['authors'][0]['initials'], 'J.' );
	...
	$expected_authors = array( array( 'surname' => 'Smith', 'initials' => 'J.', 'fullName' => 'John Smith' ) );
	$expected_fields  = array( 'year' => '2019', 'title' => 'Book One', 'publisher' => 'Routledge' );
	$expected_built   = Citex_APA_Book_Dragdrop_Parts::build( $candidate['dragdropPartKeys'], $expected_authors, $expected_fields );
	check( '[1] Question Parts are exactly Citex_APA_Book_Dragdrop_Parts::build()\'s own output', $candidate['questionParts'], $expected_built['parts'] );
	check( '[1] Fixed Text is exactly Citex_APA_Book_Dragdrop_Parts::build()\'s own output', $candidate['fixedText'], $expected_built['fixedText'] );
	check( '[1] confusingWords is exactly Citex_APA_Book_Dragdrop_Parts::build()\'s own output', $candidate['confusingWords'], $expected_built['confusingWords'] );
	check( '[1] reconstructedReference matches build_reference()', $candidate['reconstructedReference'], 'Smith, J. (2019). Book One. Routledge.' );
...
$batch_ids   = array();
for ( $i = 1; $i <= 30; $i++ ) {
	$batch_items[] = apa_dragdrop_item( array( 'John Smith' ), (string) $i, diverse_publisher( $i ) );
	$batch_ids[]   = 'AB' . str_pad( $i, 2, '0', STR_PAD_LEFT );
}
...
// ---------------------------------------------------------------------
// 3. Multi-author records (2, 3, 4 authors — APA always lists everyone in
// full, joined with "&") generate and validate correctly too — the
// "ampersand" candidate is exercised across a spread of seeds for each
// count.
// ---------------------------------------------------------------------
function check_multi_author( $section, $author_full_names ) {
	...
	$ids   = array();
	for ( $i = 1; $i <= 20; $i++ ) {
		$items[] = apa_dragdrop_item( $author_full_names, $section . $i, diverse_publisher( $i ) );
		$ids[]   = 'AB' . str_pad( $i, 2, '0', STR_PAD_LEFT ) . $section;
	}
...
// outright — APA carries no `place` field at all, so only the
// publisher-dominance check can ever fire for it.
// ---------------------------------------------------------------------
$repetitive_items = array();
$repetitive_ids   = array();
for ( $i = 1; $i <= 10; $i++ ) {
	$repetitive_items[] = apa_dragdrop_item( array( 'John Smith' ), 'R' . $i, 'Routledge' );
	$repetitive_ids[]   = 'ABR' . $i;
}
...
for ( $i = 1; $i <= 10; $i++ ) {
	$genuinely_diverse_items[] = apa_dragdrop_item( array( 'John Smith' ), 'V' . $i, diverse_publisher( $i ) );
	$genuinely_diverse_ids[]   = 'ABV' . $i;
}
