<?php
/**
 * Regression tests for Citex_Generated_Validator's Harvard in-text citation
 * wiring — validate_intext_dragdrop()/validate_intext_mcq_variant(),
 * dispatched off the `group`/`mcqPattern` fields
 * Citex_AI_V2::normalise_intext_item() now produces.
 *
 * Positive coverage: every one of the 24 combinations (4 categories x 3
 * forms x 2 types) Citex_AI_V2::normalise() produces for Harvard in-text
 * citation validates as 'passed'. Negative coverage: deliberately
 * corrupting one field at a time makes validate() FAIL with a specific
 * error code — proving the validator is actually checking content, not
 * just rubber-stamping whatever shape it is handed.
 *
 * Repo-level only, run with plain
 * `php tests/generated-validator-intext.test.php` — not shipped in
 * citex-tools.zip.
 */
...
// ---------------------------------------------------------------------
// 4. The screenshot-1 regression: a hand-corrupted narrative DragDrop
// candidate whose reconstructedReference wrongly puts the author inside
// the parentheses must fail validation, never pass.
// ---------------------------------------------------------------------
$narrative_sample = build_candidate( $BOOK, 'narrative', 'DragDrop', 9001 );
if ( ! is_wp_error( $narrative_sample ) ) {
	$broken = $narrative_sample;
	$who    = $broken['authors'][0]['fullName'] ?? '';
	$broken['reconstructedReference'] = '(' . $broken['authors'][0]['surname'] . ', ' . $broken['year'] . ') ' . $broken['clause'] . '.';
	check( '[4] the screenshot-1 mistake (author inside parens) fails validation', ( Citex_Generated_Validator::validate( $broken )['status'] ?? '?' ), 'failed' );
} else {
	check( '[4] a narrative sample was available for the regression check', false, true );
}
