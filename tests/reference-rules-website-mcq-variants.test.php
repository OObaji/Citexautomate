...
// Note: the 'author_or_organisation_format' variant ("Which option
// correctly formats the organisation's name?") was removed entirely per
// user feedback — it tested trivial name-formatting/punctuation trivia
// rather than a real Harvard referencing rule.

// ---------------------------------------------------------------------
// 8. identify_the_error: the stem embeds a reference with exactly one
...
check( '[8] the correct answer is a plain-English statement, not a reference', false !== strpos( $identify['correctAnswer'], 'Ross, T.' ), false );
check( '[8] the stem contains a broken reference for this record', false !== strpos( $identify['stem'], 'Ross, T.' ), true );

// ---------------------------------------------------------------------
// 9. CRITICAL — not_a_correct_reference: the requested variant. Its
...
check( '[9] the flawed correctAnswer does NOT match the full Harvard website format', 1 === preg_match( Citex_Reference_Rules::format_regex( Citex_Reference_Rules::CATEGORY_WEBSITE ), $not_correct['correctAnswer'] ), false );
