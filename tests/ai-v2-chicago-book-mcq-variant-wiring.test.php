<?php
/**
 * Regression tests for Citex_AI_V2's Chicago (Author-Date) Book MCQ
 * generation path — mirrors tests/ai-v2-apa-book-mcq-variant-wiring.test.php's
 * own structure, but for Chicago's own curated 8-variant catalogue
 * (Citex_Chicago_Book_Mcq_Variants): Gemini supplies only the canonical
 * record (authorFullNames/year/bookTitle/place/publisher — a `place` field
 * IS present, unlike MLA/APA), normalise_chicago_book_mcq_variant_item()
 ...
 * the entire question deterministically via
 * Citex_Chicago_Book_Mcq_Variants::build().
 *
 * Repo-level only, run with plain
 * `php tests/ai-v2-chicago-book-mcq-variant-wiring.test.php` — not shipped
 * in citex-tools.zip.
 */
...
function invoke_normalise( $questions, $ids, $target_count = null ) {
	...
	return $reflection->invoke( null, $questions, $ids, 'medium', array(), 'MCQ', Citex_Reference_Rules::CATEGORY_BOOK, $target_count, '', '', 'full_reference', 'chicago' );
}

function chicago_mcq_item( $author_names, $suffix, $place = 'London', $publisher = 'Routledge' ) {
	return array(
...
// Deliberately different-length pools — see ai-v2-chicago-book-dragdrop-wiring.test.php's
...
$known_variants = Citex_Chicago_Book_Mcq_Variants::variants();

// ---------------------------------------------------------------------
// 1. A single-author question is fully Citex-authored: source/category,
// mcqPattern, chicagoBookMcqVariant, and an exact match against
// Citex_Chicago_Book_Mcq_Variants::build()'s own output for that variant
// and this record's fields.
// ---------------------------------------------------------------------
$single_item   = chicago_mcq_item( array( 'John Smith' ), 'One', 'London', 'Routledge' );
$single_result = invoke_normalise( array( $single_item ), array( 'CB01' ) );
check( '[1] normalise() succeeds for a single-author Chicago Book MCQ item', is_wp_error( $single_result ), false );
if ( ! is_wp_error( $single_result ) ) {
	...
	check( '[1] mcqPattern is chicago_book_mcq_variant', $candidate['mcqPattern'], 'chicago_book_mcq_variant' );
	check( '[1] chicagoBookMcqVariant is one of the 8 known variants', in_array( $candidate['chicagoBookMcqVariant'], $known_variants, true ), true );
	check( '[1] no apaBookMcqVariant field is carried at all', array_key_exists( 'apaBookMcqVariant', $candidate ), false );
	...
	$expected = Citex_Chicago_Book_Mcq_Variants::build(
		$candidate['chicagoBookMcqVariant'],
...
$batch_ids   = array();
for ( $i = 1; $i <= 30; $i++ ) {
	$batch_items[] = chicago_mcq_item( array( 'John Smith' ), (string) $i, diverse_place( $i ), diverse_publisher( $i ) );
...
if ( ! is_wp_error( $batch_result ) ) {
	$variants_seen = array_unique( array_column( $batch_result, 'chicagoBookMcqVariant' ) );
	...
	foreach ( $batch_result as $candidate ) {
		if ( ! in_array( $candidate['chicagoBookMcqVariant'], $known_variants, true ) ) {
			check( '[2] every candidate\'s chicagoBookMcqVariant is a known variant', $candidate['chicagoBookMcqVariant'], '(unknown)' );
		}
	}
}
...
function check_variant_compatible_with_count( $section, $author_full_names, $target_count ) {
	...
	$ids   = array();
	for ( $i = 1; $i <= 20; $i++ ) {
		$items[] = chicago_mcq_item( $author_full_names, (string) $i, diverse_place( $i ), diverse_publisher( $i ) );
	...
	foreach ( $result as $candidate ) {
		$bounds = Citex_Chicago_Book_Mcq_Variants::variant_author_requirement( $candidate['chicagoBookMcqVariant'] );
...
// to Journal Article's field-variety mechanism) — Chicago Book MCQ
// candidates never carry it.
// ---------------------------------------------------------------------
if ( ! is_wp_error( $single_result ) ) {
	check( '[4] Chicago Book MCQ candidates carry no exerciseDesign field at all', array_key_exists( 'exerciseDesign', $single_result[0] ), false );
}
