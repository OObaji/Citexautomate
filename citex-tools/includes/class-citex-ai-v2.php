...
class Citex_AI_V2 {
	...
	private static function journal_article_design_prompt_note( $exercise_design ) {
		$notes = array(
			...
			'author_year_volume_pages' => "EXERCISE DESIGN — AUTHOR(S) + YEAR + VOLUME + PAGES (exactly 4 draggable parts):\n- This question shows and tests the author list AS ONE COMPACT PIECE (all authors joined together — never one piece per author, never \"et al.\"), plus the year, volume and page range as 3 further short pieces — 4 pieces total, never the article title or journal title. Still provide the complete real record above, but prefer author names, volume and page range that stay short.",
			...
			'year_volume_issue_pages'  => "EXERCISE DESIGN — YEAR + VOLUME + ISSUE + PAGES, NO AUTHOR (exactly 4 draggable parts):\n- This question shows and tests the year, volume, issue and page range as 4 short pieces — not the author(s), article title or journal title at all. Still provide the complete real record above.",
		);
	...
	private static function normalise_journal_article_item( $item, $id, $authors, $year, $article_title, $journal_title, $volume, $issue, $pages, $scenario, $exercise, $difficulty, $expected_distractors, $exercise_design = 'full_reference' ) {
		...
		// precisely because neither can satisfy the hard 3-4-part DragDrop
		// rule; reject outright if one is ever assigned to a DragDrop slot
