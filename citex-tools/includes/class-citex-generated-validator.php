...
class Citex_Generated_Validator {
	...
	public static function validate( $question ) {
		if (
			! in_array( (string) ( $question['source'] ?? '' ), array( 'Harvard', 'MLA', 'APA', 'Chicago' ), true ) ||
			...
			return self::result(
				'failed',
				array(
					self::error( 'UNSUPPORTED_GENERATED_FORMAT', 'Generated validation currently supports only Harvard / ReferenceList / Book, Edited Book, Journal Article or Website, DragDrop or MCQ, or MLA / ReferenceList / Book, DragDrop or MCQ, or APA / ReferenceList / Book, DragDrop or MCQ, or Chicago / ReferenceList / Book, DragDrop or MCQ, or Harvard/MLA / InTextCitation / any category / DragDrop or MCQ.' ),
				),
				null
			);
		}
	...
	private static function validate_dragdrop( $question ) {
		...
		$style     = 'MLA' === $source ? 'mla' : ( 'APA' === $source ? 'apa' : ( 'Chicago' === $source ? 'chicago' : 'harvard' ) );
	...
	private static function expected_designation_for( $question, $category ) {
		...
		// Citex_MLA_Reference_Rules::join_editors()'s own docblock) — its
		// editor records also carry `givenName`, not Harvard's `initials`,
		// so this Harvard-only helper must never run against one (it would
		// otherwise emit a PHP warning reading the missing `initials` key
		// several calls downstream, even though the resulting check is a
		// harmless no-op either way).
		if ( Citex_Reference_Rules::CATEGORY_EDITED_BOOK !== $category || 'MLA' === (string) ( $question['source'] ?? '' ) ) {
			return null;
		}
		$editors = is_array( $question['editors'] ?? null ) ? $question['editors'] : array();
		if ( empty( $editors ) ) {
	...
	private static function expected_editor_join_for( $question, $category ) {
		// Same MLA guard as expected_designation_for() — MLA editor
		// records carry `givenName`, not Harvard's `initials`, and MLA's
		// own joining rule (see Citex_MLA_Reference_Rules::join_people())
		// is already fully enforced by the exact-match reconstruction
		// checks elsewhere, so this Harvard-only helper has nothing to add
		// for MLA and must never read the missing `initials` key.
		if ( Citex_Reference_Rules::CATEGORY_EDITED_BOOK !== $category || 'MLA' === (string) ( $question['source'] ?? '' ) ) {
			return null;
		}
		$editors = is_array( $question['editors'] ?? null ) ? $question['editors'] : array();
	...
	private static function validate_reference_format( $reference, $category = null, $place = null, $publisher = null, $expected_designation = null, $expected_editor_join = null, $exercise_design = null, $style = 'harvard' ) {
		...
		$format_regex = $is_mla
			...
			: ( $is_apa ? Citex_APA_Reference_Rules::format_regex( $category ) : ( $is_chicago ? Citex_Chicago_Reference_Rules::format_regex( $category ) : Citex_Reference_Rules::format_regex( $category, $exercise_design ) ) );
