...
/**
 * Chicago (Author-Date) in-text citation DragDrop builder — mirrors
 * Citex_Intext_Dragdrop_Parts's own "always draw every blank, never a
 * subset" model exactly, but with Chicago's OWN no-comma-before-year
 * token layout (see Citex_Chicago_Intext_Citation_Rules's own docblock:
 * "(Who Year)", never "(Who, Year)"; "(Who Year, Page)" with no "p."
 * prefix for a direct quote).
 *
 ...
 */
class Citex_Chicago_Intext_Dragdrop_Parts {

	/**
	 * The full ordered token list for one Chicago in-text citation
	 * sentence.
	 *
	...
	/**
	 * Builds {parts, fixedText, confusingWords} for one Chicago in-text
	...
	private static function distractor_for( $kind, $value, array $surnames, $who, $record_seed ) {
		switch ( $kind ) {
			case 'who':
				return Citex_Chicago_Intext_Citation_Rules::wrong_join( $surnames, $who );
			case 'year':
