...
class Citex_Admin {
	...
	public function register_menu() {
		$this->page_hooks[] = add_menu_page( __( 'Citex', 'citex-tools' ), __( 'Question generator', 'citex-tools' ), 'manage_options', 'citex', array( $this->dashboard, 'render' ), 'dashicons-book-alt', 30 );
	...
	public function enqueue_assets( $hook ) {
		...
		wp_localize_script( 'citex-admin', 'citexTools', array(
			...
			'detectCitationsAction' => Citex_Scanner::AJAX_DETECT_CITATIONS,
			'strings'            => array(
				...
				'detecting'      => __( 'Looking for the Citations post type…', 'citex-tools' ),
				'detected'       => __( 'Found it: %s. Saved.', 'citex-tools' ),
				'detectFailed'   => __( 'Could not find it automatically:', 'citex-tools' ),
			...
			'generator' => array(
				...
				'strings'              => array(
					...
					'targetReached'   => __( '✓ Done — target reached, {total}/{target} published.', 'citex-tools' ),
	...
	public function render_notice() {
		...
		// A real requested feature: a small visual tick on a genuinely
		// successful action (e.g. "Generate & Publish" completing) so the
		// admin has an unambiguous "this finished" signal at a glance —
		// not just the notice's own colour, which is easy to miss after
		// switching tabs while a batch runs. Never added to a
		// warning/error/info notice, which already read as "needs your
		// attention" without one.
		$prefix = 'success' === $type ? '✓ ' : '';
		echo '<div class="notice notice-' . esc_attr( $type ) . ' is-dismissible citex-action-notice"><p><strong>Citex:</strong> ' . esc_html( $prefix . $notice['message'] ) . '</p></div>';
	}
