...
/**
 * MHRA in-text citation MCQ catalogue — mirrors Citex_Intext_Mcq_Variants
 * exactly (see Citex_MHRA_Intext_Citation_Rules's own docblock for why
 * MHRA's In-Text Citation deliberately targets the same shape Harvard
 * already uses), via its own genuinely separate sibling classes throughout.
 *
 * Same crc32-seeded per-question variant selection, same exact-match
 * validator story, same 4-variant catalogue gated by citation form and
 * author count as every other in-text MCQ class.
 *
 * Pure and static, no WordPress/ACF calls — unit-testable directly.
 */
class Citex_MHRA_Intext_Mcq_Variants {
	...
	public static function variant_form_requirement( $variant ) {
		if ( 'page_punctuation' === $variant ) {
			return array( Citex_MHRA_Intext_Citation_Rules::FORM_PARENTHETICAL_QUOTE );
		}
		return null;
	}
	...
	public static function variant_for( $seed, $form, $author_count ) {
		...
		$index = abs( crc32( 'mhra_intext_mcq_variant|' . (string) $seed ) ) % count( $compatible );
	...
	private static function full_sentence( array $fields ) {
		...
		if ( Citex_MHRA_Intext_Citation_Rules::FORM_NARRATIVE === $form ) {
			return Citex_MHRA_Intext_Citation_Rules::narrative_sentence( $fields['who'], $fields['year'], $fields['clause'] );
		}
		if ( Citex_MHRA_Intext_Citation_Rules::FORM_PARENTHETICAL === $form ) {
			return Citex_MHRA_Intext_Citation_Rules::parenthetical_sentence( $fields['who'], $fields['year'], $fields['clause'] );
		}
		return Citex_MHRA_Intext_Citation_Rules::parenthetical_quote_sentence( $fields['who'], $fields['year'], $fields['page'], $fields['quote'] );
	}
	...
	private static function broken_sentence( $kind, array $fields ) {
		...
		switch ( $kind ) {
			...
			case 'wrong_who':
				return self::full_sentence( array_merge( $fields, array( 'who' => Citex_MHRA_Intext_Citation_Rules::wrong_join( $fields['surnames'], $who ) ) ) );
			case 'wrong_year':
				$wrong_year = Citex_Reference_Rules::year_distractor( (string) $fields['year'], $who . '|' . $fields['year'] . '|mhra_intext_mcq_year' );
	...
	private static function pick_error_kind( array $fields, $salt = '' ) {
		...
		return $kinds[ abs( crc32( 'mhra_intext_mcq_error_kind|' . $record_seed ) ) % count( $kinds ) ];
	}
	...
	private static function build_correct_format( array $fields ) {
		...
		if ( Citex_MHRA_Intext_Citation_Rules::FORM_PARENTHETICAL_QUOTE === $form ) {
			$kinds = array( 'missing_comma', 'missing_page', 'missing_p_prefix' );
		} else {
			$kinds = array(
				Citex_MHRA_Intext_Citation_Rules::FORM_NARRATIVE === $form ? 'author_inside_parens' : 'missing_comma',
				'wrong_who',
				'wrong_year',
			);
		}
		return array(
			'stem'          => Citex_MHRA_Intext_Citation_Rules::mcq_question_stem( $form ),
	...
	// in-text-only "et al. at 4+" threshold directly.
	// -----------------------------------------------------------------
	private static function build_join_convention( array $fields ) {
		...
		if ( $count >= 4 ) {
			...
			$last = array_pop( $copy );
			$wrong_whos = array(
				implode( ', ', $copy ) . ' and ' . $last,
				$surnames[0] . ' et al',
				$surnames[0] . ' and others',
			);
		} else {
			$wrong_whos = array(
	...
	// -----------------------------------------------------------------
	// Variant 3 — Page Punctuation (parenthetical_quote only).
	// -----------------------------------------------------------------
