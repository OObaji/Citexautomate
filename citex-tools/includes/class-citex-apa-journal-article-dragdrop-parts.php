...
/**
 * APA Journal Article DragDrop's dynamic question builder — mirrors
 * Citex_MLA_Journal_Article_Dragdrop_Parts's own "select 3 of a content
 * pool, always split the first author into surname/initials" model exactly,
 * applied to APA's own Journal Article rule (see
 * Citex_APA_Reference_Rules's own docblock): NO quotation marks around the
 * sentence-case article title (unlike Harvard's single quotes and MLA's
 * double quotes), a bare "Volume(Issue)" (Harvard's own shorthand, never
 * MLA's "vol."/"no." labels), and — critically — NO "pp." prefix before the
 * page range, unlike Harvard.
 *
 * The author-segment tokens (surname/initials split, "&" joiner with a
 * comma before it even at exactly two authors) mirror
 * Citex_APA_Edited_Book_Dragdrop_Parts's own editor-segment tokens — APA's
 * author-joining rule does not vary by category.
 *
 * Pure and static, no WordPress/ACF calls — unit-testable directly.
 */
class Citex_APA_Journal_Article_Dragdrop_Parts {

	/**
	 * @return string[]
	 *
	 * 'author_name' costs 2 concrete parts (surname + initials together);
	 * every other slot here costs 1. There is no `publisher` slot at all —
	 * Journal Article has no publisher/place element.
	 */
	...
	/**
	 * Builds the full ordered token list for one APA journal article record.
	 * The author-segment tokens are identical in shape to
	 * Citex_APA_Edited_Book_Dragdrop_Parts::build_tokens()'s own leading
	 * segment (minus the designation); the page range renders via
	 * Citex_APA_Reference_Rules::format_page_range() — the same en-dash
	 * conversion every other category's page range already uses — with NO
	 * "pp." prefix (this category's single most APA-distinctive rule).
	 *
	...
	public static function build_tokens( array $authors, array $fields ) {
		...
		$tokens[] = array( 'key' => 'author_initials', 'kind' => 'author_initials', 'value' => (string) $first['initials'], 'literal' => false );

		if ( 1 === $count ) {
			$tokens[] = array( 'key' => null, 'kind' => 'literal', 'value' => ' (', 'literal' => true );
		} else {
			$middle      = array_slice( $authors, 1, $count - 2 );
			$middle_text = '';
			foreach ( $middle as $person ) {
				$middle_text .= sprintf( ', %s, %s', $person['surname'], $person['initials'] );
			}
			$last     = $authors[ $count - 1 ];
		...
		$tokens[] = array( 'key' => null, 'kind' => 'literal', 'value' => '), ', 'literal' => true );
		$tokens[] = array( 'key' => 'pages', 'kind' => 'pages', 'value' => Citex_APA_Reference_Rules::format_page_range( $fields['pages'] ), 'literal' => false );
		$tokens[] = array( 'key' => null, 'kind' => 'literal', 'value' => '.', 'literal' => true );
		return $tokens;
	}
	 ...
	 * id — identical technique to
	 * Citex_MLA_Journal_Article_Dragdrop_Parts::select_parts().
	 *
	...
	public static function select_parts( $seed, array $authors ) {
		...
		$seed_slot     = $content_slots[ abs( crc32( 'apa_journal_article_dragdrop_seed|' . $seed ) ) % count( $content_slots ) ];
		...
		usort(
			...
			function ( $a, $b ) use ( $seed ) {
				$hash_a = crc32( 'apa_journal_article_dragdrop_shuffle|' . $seed . '|' . $a );
				$hash_b = crc32( 'apa_journal_article_dragdrop_shuffle|' . $seed . '|' . $b );
		...
		foreach ( $selected_abstract as $slot ) {
			if ( 'author_name' === $slot ) {
				...
				$selected_concrete[] = 'author_initials';
			} else {
	...
	private static function distractor_for( $kind, $value, array $authors, array $fields, $record_seed ) {
		switch ( $kind ) {
			...
			case 'author_initials':
				return self::author_initials_distractor( $value, $authors, $record_seed );
			case 'ampersand':
				return 'and';
			case 'year':
	...
	private static function author_surname_distractor( $value, array $authors, $record_seed ) {
		...
		if ( null !== $other && 0 === ( abs( crc32( 'apa_journal_article_dragdrop_surname_flavor|' . $record_seed ) ) % 2 )
	...
	private static function author_initials_distractor( $value, array $authors, $record_seed ) {
		$other = isset( $authors[1] ) ? $authors[1] : null;
		if ( null !== $other && 0 === ( abs( crc32( 'apa_journal_article_dragdrop_initials_flavor|' . $record_seed ) ) % 2 )
	...
	private static function pages_distractor( $value, $record_seed ) {
		...
		$delta = 1 + ( abs( crc32( 'apa_journal_article_dragdrop_pages|' . $record_seed ) ) % 9 );
