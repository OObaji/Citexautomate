...
/**
 * MHRA Book DragDrop's dynamic 3-part question builder — mirrors
 * Citex_Chicago_Book_Dragdrop_Parts's own "always the full reference, N
 * random blanks" model, but for MHRA's own rule (see
 * Citex_MHRA_Reference_Rules's own docblock): the FULL given name (never an
 * initial), only the FIRST author is inverted ("Surname, First") — every
 * author AFTER the first is rendered in NATURAL word order ("First
 * Surname") instead, so the drawn author's surname/given-name tokens swap
 * their relative order depending on whether the drawn author is first or
 * later — every author always listed in full (never "et al."), `place` IS
 * a candidate (unlike MLA/APA, like Harvard/Chicago), and place, publisher
 * AND year all sit together inside ONE parenthesis (unlike Chicago, which
 * keeps the year in its own separate segment outside the place/publisher
 * parenthesis).
 *
 * Eligible content candidates: author_name (costs 2: surname + given name),
 * title, place, publisher, year — the same 5-slot set Chicago Book uses.
 * The structural "and" candidate (correct value "and") is eligible only
 * when there are 2+ authors, exactly mirroring Chicago/Harvard Book's own
 * structural-joiner eligibility rule.
 *
 ...
 * literal, folded in at its own position (inverted if first, natural word
 * order otherwise) exactly as it would appear in the full reference —
 * identical in spirit to every other style's own "drawn author index"
 ...
 * MHRA-specific, are written here directly.
 *
 * Pure and static, no WordPress/ACF calls, exactly like
 * Citex_Reference_Rules/Citex_MHRA_Reference_Rules and
 * Citex_Chicago_Book_Dragdrop_Parts — unit-testable directly.
 */
class Citex_MHRA_Book_Dragdrop_Parts {
	...
	private static function content_slots() {
		return array( 'author_name', 'title', 'place', 'publisher', 'year' );
	}
	...
	/**
	 * Builds the full ordered token list for one MHRA book record. The
	 * drawn author's surname/given-name tokens appear in INVERTED order
	 * ("Surname, ||") when $drawn_author_index is 0, but in NATURAL order
	 * ("||, given-name-first, then surname" — i.e. given name token, then a
	 * literal space, then surname token) for any later index — reflecting
	 * MHRA's own "only the first author is inverted" rule exactly, so a
	 * later author drawn into parts still reconstructs in the correct word
	 * order. Every literal (non-drawn) author is rendered the same way:
	 * "Surname, Given" at index 0, "Given Surname" at any later index.
	 *
	 * Place, publisher and year all sit inside ONE trailing parenthesis
	 * (unlike Citex_Chicago_Book_Dragdrop_Parts's own build_tokens(), which
	 * keeps the year in its own separate segment) — the single most
	 * MHRA-distinctive structural difference from Chicago's own token
	 * stream.
	 *
	 ...
	 * id — same technique as Citex_Chicago_Book_Dragdrop_Parts::select_parts().
	 *
	...
	public static function select_parts( $seed, array $authors ) {
		...
		$drawn_index  = abs( crc32( 'mhra_book_dragdrop_author|' . $seed ) ) % max( 1, $author_count );
		$target_count = 3;

		$content_slots = self::content_slots();
		$seed_slot     = $content_slots[ abs( crc32( 'mhra_book_dragdrop_seed|' . $seed ) ) % count( $content_slots ) ];
		...
		usort(
			...
			function ( $a, $b ) use ( $seed ) {
				$hash_a = crc32( 'mhra_book_dragdrop_shuffle|' . $seed . '|' . $a );
				$hash_b = crc32( 'mhra_book_dragdrop_shuffle|' . $seed . '|' . $b );
		...
		$ordered = array();
		foreach ( self::build_tokens( $authors, array( 'title' => '', 'place' => '', 'publisher' => '', 'year' => '' ), $drawn_index ) as $token ) {
	 ...
	 * Citex_Chicago_Book_Dragdrop_Parts::build().
	 *
	...
	public static function build( array $selected_keys, array $authors, array $fields ) {
		...
		$record_seed  = implode( '|', array( (string) $fields['title'], (string) $fields['place'], (string) $fields['publisher'], (string) $fields['year'], (string) count( $authors ) ) );
	...
	private static function distractor_for( $kind, $value, array $authors, $drawn_index, array $fields, $record_seed ) {
		switch ( $kind ) {
			...
			case 'and':
				// Tests "authors after the first are joined with the word
				// 'and', never '&'".
				return '&';
			case 'title':
	 ...
	 * every other style's own Book Dragdrop class: attributing the OTHER
	 * author's surname (only eligible with 2+ authors), or a wrongly
	 * possessive form as the single-author fallback.
	 */
	private static function author_surname_distractor( $value, array $authors, $drawn_index, $record_seed ) {
		...
		if ( null !== $other && 0 === ( abs( crc32( 'mhra_book_dragdrop_surname_flavor|' . $record_seed ) ) % 2 )
	 ...
	 * "the FULL given name is required, never an initial").
	 */
	private static function author_givenname_distractor( $value, array $authors, $drawn_index, $record_seed ) {
		$other = self::other_author( $authors, $drawn_index );
		if ( null !== $other && 0 === ( abs( crc32( 'mhra_book_dragdrop_givenname_flavor|' . $record_seed ) ) % 2 )
