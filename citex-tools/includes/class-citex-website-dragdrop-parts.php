 ...
 * and website_dragdrop_shape(), both now dead/legacy code, superseded by
 * this class and by Citex_Website_Mcq_Variants) with the same "always the
 * full reference, N random blanks" model Book DragDrop already uses (see
 * Citex_Book_Dragdrop_Parts): the complete reference — "Author/Org (Year)
 * Title. Available at: URL (Accessed: Day Month Year)." — is always
 * rendered in full, and exactly 3 of 4 eligible fields are seeded-randomly
 * chosen as draggable blanks (author, year, title, accessed date — `url`
 * is always shown as literal text, never draggable; see content_slots()'s
 * own docblock for why); every other field stays visible as literal text.
 * There is no publisher element in this format at all — the record's own
 * `publisher` field, if present, is simply not used here.
 *
 ...
 * filler word like "and" — all 4 eligible candidates are pure content, so
...
class Citex_Website_Dragdrop_Parts {
	 ...
	 * text; it just never becomes one of the 3 draggable parts. There is no
	 * `publisher` slot at all — this format never shows a publisher.
	 */
	private static function content_slots() {
		return array( 'author', 'year', 'title', 'accessed_date' );
	}
	...
	public static function build_tokens( array $author, array $fields ) {
		return array(
			...
			array( 'key' => null, 'kind' => 'literal', 'value' => '. Available at: ', 'literal' => true ),
	...
	public static function select_parts( $seed ) {
		...
		$empty_fields = array( 'year' => '', 'title' => '', 'url' => '', 'accessedDate' => '' );
		$ordered      = array();
