		...
		<table class="wp-list-table widefat fixed striped citex-table">
			...
			<?php foreach ( $pending_questions as $question ) : ?>
				...
				<tr>
					...
					<td><?php $is_identify_error = 'identify_error' === (string) ( $question['mcqPattern'] ?? '' ); ?><?php if ( $is_identify_error ) : ?><span class="description"><?php esc_html_e( 'Identify the error:', 'citex-tools' ); ?></span><br /><?php endif; ?><?php echo $is_identify_error ? nl2br( esc_html( $question['scenario'] ?? '' ) ) : esc_html( $question['scenario'] ?? '' ); ?></td>
					<td>
						<?php if ( 'MCQ' === $q_type ) : ?>
							<?php $option_reasons = (array) ( $question['optionErrorReasons'] ?? array() ); ?>
							<strong><?php echo $is_identify_error ? esc_html__( 'True description:', 'citex-tools' ) : esc_html__( 'Answer:', 'citex-tools' ); ?></strong> <?php echo esc_html( $question['reconstructedReference'] ?? '' ); ?>
