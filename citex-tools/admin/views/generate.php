		...
		<table class="wp-list-table widefat fixed striped citex-table">
			...
			<?php foreach ( $pending_questions as $question ) : ?>
				...
				<tr>
					...
					<td><?php $mcq_pattern = (string) ( $question['mcqPattern'] ?? '' ); $is_identify_error = 'identify_error' === $mcq_pattern; $is_choose_treatment = 'choose_treatment' === $mcq_pattern; ?><?php if ( $is_identify_error ) : ?><span class="description"><?php esc_html_e( 'Identify the error:', 'citex-tools' ); ?></span><br /><?php elseif ( $is_choose_treatment ) : ?><span class="description"><?php esc_html_e( 'Choose the correct rule:', 'citex-tools' ); ?></span><br /><?php endif; ?><?php echo $is_identify_error ? nl2br( esc_html( $question['scenario'] ?? '' ) ) : esc_html( $question['scenario'] ?? '' ); ?></td>
					<td>
						<?php if ( 'MCQ' === $q_type ) : ?>
